...
list.of.packages <-
  c(
    ...
    # time series
    "lubridate", "zoo","tseries","timeDate",'seasonal',
...
mortgage <- function(P=500000, I=6, L=30, amort=T, plotData=T, PRINT = F ) { 
...
ChineseNewYear = 
  structure(c(-14198, -13844, -13489, -13105, -12750, -12396, -12012, 
              -11658, -11274, -10920, -10566, -10182, -9828, -9473, -9088, 
              -8735, -8380, -7996, -7642, -7258, -6904, -6549, -6165, -5811, 
              -5456, -5072, -4718, -4335, -3980, -3626, -3242, -2887, -2533, 
              -2149, -1794, -1441, -1057, -702, -318, 36, 391, 775, 1129, 1483, 
              1867, 2221, 2605, 2959, 3314, 3698, 4053, 4407, 4791, 5145, 5529, 
              5883, 6237, 6621, 6976, 7331, 7715, 8069, 8423, 8806, 9161, 9545, 
              9899, 10254, 10638, 10992, 11346, 11730, 12084, 12439, 12823, 
              13177, 13562, 13916, 14270, 14654, 15008, 15362, 15746, 16101, 
              16484, 16839, 17194, 17577, 17932, 18286, 18670, 19024, 19379, 
              19763, 20117, 20501, 20855, 21209, 21593, 21948), class = "Date")


WeekName_abbr = c('Sun','Mon','Tue','Wed','Thu','Fri','Sat')
WeekName_full = c('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday') 
...
week_counting = function(date_series, 
                         start_date = NULL, # starting date for the first full week of this year or next year
                         PRINT = F){
  diff_days = (as.Date(date_series) - as.Date(start_date)) %>% as.numeric
  result = floor((diff_days %>% as.numeric-0.01)/7+1.01)
  
  return(result)
  
  if (F) {
    
    date_series =  c('2013-1-5',"2013-1-6",
                     "2014-12-23","2014-12-25","2014-12-28","2014-12-31",
                     "2015-1-1","2015-1-3","2015-1-6",
                     "2015-7-1",'2015-7-25','2015-7-26',
                     '2016-12-31',
                     '2017-01-01','2017-01-07') %>% as.Date
    
    start_date = '2012-12-30' %>% as.Date
    
    week_counting(date_series, start_date)
  }
}


week_counting_df = function(date_series, 
                      start_date = NULL, # starting date for the first full week of this year or next year
                      PRINT = F){ 
  # an attach_Load_First.R function
  
  # this function will return the week information for data series
  
  
  if (is.na(date_series) %>% sum) stop('clean NAs before you run')
  
  if (is.null(start_date)){
    start_date = min(date_series,na.rm = T)
  }
  
  # date_series %>% range
  diff_days = (as.Date(date_series) - as.Date(start_date)) %>% as.numeric
  
  week_count_cycle = week_count = week_counting(date_series, start_date)
  
  # df_week_count_cycle = data.frame(date_series,week_count_cycle)
  
  for (YY in unique(year(date_series)) ){
    # print(YY)
    
    YY_postions = year(date_series) == YY
    
    begin_year = paste(YY,'-01-01',sep='') %>% as.Date
    
    begin_year_week_count_cycle = week_counting(begin_year, start_date)

    if (begin_year_week_count_cycle>=52) {
      weeks_to_minus = begin_year_week_count_cycle - 1
      week_count_cycle[YY_postions] = week_count_cycle[YY_postions] - weeks_to_minus
    }
  }
  
  # range(week_count_cycle)
  Output = data.frame(week_count_cycle,
                      weekday = weekdays(date_series,abbreviate = T), week_count, date_series, diff_days, start_date)

  if (sum(Output$week_count_cycle > 53)) stop("counting weeks are wrong")
  ...
  if (F){
    ...
    week_counting_df(date_series, start_date)
    
    
    date_series = '2017-01-07' %>% as.Date
    start_date = '2012-12-30' %>% as.Date
...
Join = function(x, y, by = NULL, type = "left", match = "all",
  ...
  # 'replace' means we replace values in dup x only if at the same row there is a value in dup y
    ...
    for ( i in dup_names){
      ...
      if (duplicate =='replace' ){
        ...
        if (
          ...
          if (PRINT) {
            message(colnames(test)[dup_position[1]], " will be replace by y at places: ")
            print(test[y_places,c(by,i)])
          }
  ...
  if (F){
    ...
    Join (Table1,Table2[1:2,],by = c('A'),type = 'left',duplicate = 'replace')
...
ldply0 = function(x, # data
  ...
  if (sum(index %in% colnames(x))< length(index)) {
    stop ('index cannot be found in colnames') 
  }
...
setvalues.copy = function(data, variable, value_origin, value_new){
  ...
  if ('factor' %in% class(data[,variable])){
    stop ('cannot assign values to factor') 
  }
...
deletenames = function(x,
  ...
  # output the dataset/ named vector after delete certain vars
  ...
  if (fuzzy) {
    postion = str_detect(names(x) ,to_be_delete)
  } else {
    postion = names(x) %in% to_be_delete
  }
  
  if ('data.frame' %in% class(x)) {
    y = x[,!postion,drop = F] # drop = F is to prevent one-column dataset becoming a series
  } else {
    y = x[!postion]
  }
  
  # if x contains duplcated names, then those duplicated names will be changed in y in the above step
  # so the step below will just replace names in y with the original names in x
  names(y) = names(x)[!postion]
  
  if (PRINT){
    if (sum(postion)){
      message(
        paste(names(x)[postion],collapse=', ')," :  deleted"
      )
    } else {
  ...
  if (F) {
    ...
    # only one column left --- still a data.frame
    deletenames(diamonds[,1:2] %>% head,to_be_delete = c("cut"))

    
    # also works for named vector
    string_named_test = c(1:ncol(diamonds))
    names(string_named_test) = colnames(diamonds)
    
    deletenames(string_named_test,to_be_delete = c("cut"))
...
sanity_check = function(x ,
  ...
  if (check_vec_meaningful(exact_in_match) ){
    ...
    if (is.vector(x) && sum(x %in% exact_in_match)!=length(x)  ){
      ...
      get_unmatched(x, exact_in_match) %>% print
    ...
    if (
      ...
      get_unmatched(colnames(x), exact_in_match) %>% print
    }
  }
...
stacked_bar_prepare = function(data,
                               ...
                               grouping_var_vertical, # names to divide bars vertically
                               bar_wide = 0.5,
...
get_x_for_bar_chart = function(data, KEY_group, gap_between_gap = 3, step = 1){
  
  data[,'x'] = 0
  for (i in 2:(nrow(data))){
    
    if ( data[i,KEY_group]!= data[i-1,KEY_group]) {
      STEP =  gap_between_gap
    } else{
      STEP = step
    }
    data[i,'x'] = STEP + data[i-1,'x']
  }
  data
}
...
### Show and Output Fuctions --------------------------


show_table_as_comment = function(data,
                                 comment = '', # as title 
                                 key_position = 1, # the column position which key locates, 
                                 key_name = NULL, # the column name you want to use as key, it will overwrite key_position.
                                 markdown = F # whether to output as markdown format
                                 ){
  # show the table using a nice format which you can copy paste as text comments
  
  
  message(comment,'\n')
  
  pure_text_envir_space = ''
  addition_n = ''
  ...
  if (!is.null(key_name)){
    key_position = which( colnames(data) == key_name)[1]
  }
  
  for (i in 1:nrow(data)){
    # i = 1
    key = data[i,key_position]
    message('\n',paste(key,":",sep=''),addition_n)
    
    for (j in 1:ncol(data)){
      if (j!=key_position){
        message(pure_text_envir_space,
                rep(" ",str_length(key)) %>% paste(.,collapse = '',sep=''),
                colnames(data)[j],": ",
                data[i,j], addition_n
        )
      }
  ...
  if (F){
    ...
    iris %>% head(.,3) %>% show_table_as_comment(.,comment = 'iris dataset head', key_name = 'Species')
    iris %>% head(.,3) %>% show_table_as_comment(.,comment = 'iris dataset head', key_name = 'Species',markdown = T)
    
  }
}


show_kable_splitted = function(data = iris,  
                               KEY,comment = '', 
                               show_key_in_table = F,
                               row.names = F,
                               which_key_in_title = NULL,
                               ...){
  
  test = ldply0(data , index = KEY , func = function(data){
    
    key_pairs = data[1,KEY]
    
    if (!show_key_in_table){
      data = data[,!colnames(data) %in% KEY]
    }
    
    data %>% kable(.,
                  caption = paste(comment,paste(key_pairs,collapse = ', '), sep = ''),row.names = row.names,...) %>% print
  })
  
}
show_to_excle = function(data = head(iris), col.names = T,  row.names = F){
  ## input is a DF, the output of this function can be drectly copied to excel
  
  if (row.names == F){
    return(
      write.table(data,sep="\t",col.names = col.names, row.names = row.names)
    )
  } else {
    # if there is a index, then treat the index as an aditional data column
    data2 = cbind('.' = rownames(data),data)   
    colnames(data2)[1] = ''
    write.table(data2,sep="\t",col.names = col.names, row.names = F)
  }
  ...
  if (F) {
    ...
    rownames(test) = paste(c('test'),1:6)
    
    show_to_excle(test,row.names = T)
  }
  
}


show_as_pivot = function(data,KEY, classic_style = T){
  holder = data.frame()
  data = data[order(data$parent_name),]
  
  count = 0
  for (i in unique(data[,KEY])){
    
    if (!classic_style){
      title_df = data.frame(i)
      colnames(title_df) = eval(KEY) 
      new_data = data[data[,KEY] == i,colnames(data)!=KEY]
    } else {
      title_df = data.frame('.')
      colnames(title_df) = eval(KEY) 
      new_data = data[data[,KEY] == i,]
    }
    if (count==0) {
      holder = rbind.fill(holder, new_data)
    } else {
      holder = rbind.fill(holder,title_df, new_data)
    }
    count = count  + 1
  }
  holder
...
show_as_kable = function(data,...){
  
  for ( i in 1:ncol(data)){
    # i = 7
    # get_autodigits(x = data[,i])
    data[,i] = as.character(print_comma(data[,i],...))
    data[is.na(data[,i]),i] = ''
  }
  data
}
...
get_autodigits = function(x , max_digit = 2){
  
  numeric_result = x
  character_result = as.character(x)
  inter_part_length = str_length(floor(numeric_result))
  fraction_part = str_sub(character_result,inter_part_length + 1,str_length(character_result))
  
  fraction_part = sum(str_length(na.omit(fraction_part))>0)
  ...
  if ( F){
    ...
    get_autodigits(c(1212,12)) 
    get_autodigits(x = c(1212,12.2)) 
    get_autodigits(x = c(1212,12.2321312)) 
...
print_millions = function(x,
                          ...
                          postfix = 'm', dollar_divide = 10^6 , negative_as_bracket = F){
  # x = 9.66
  
  # dollar_divide = 1
  scipen_XXXXX = options()$scipen
  options(scipen = 999)
...
#      character_result =
      ...
      for (xi in 1:length(x)){
        # xi = 1
        if (is.na(numeric_result[xi])){ 
          character_result[xi] = ''
        } else {
          # xi = 1
          List = NULL
          
          # integer_part = as.character(floor(0.11))
          
          integer_part = as.character(floor(numeric_result[xi]))
          ...
          if (x[xi]<0 ) {
            if (negative_as_bracket) {
              character_result[xi]  = paste("(",character_result[xi] ,')',sep='',collapse = '')
            } else {
              character_result[xi]  = paste("-",character_result[xi] ,sep='',collapse = '')
            }
          } 
...
print_comma = function(x, auto_digit = T,
                       digits = 0,dollar_sign = F,NA_character = 'NA',...){
  
  
  if (is.numeric(x)){
    if (auto_digit) digits = get_autodigits(x)
    return(print_millions(x,postfix = '', dollar_divide = 1, 
                          digits = digits,
  ...
  if (F){
    ...
    print_comma(21290000000)
    print_millions(212131089321312319)
    
    print_comma(2012)
    print_comma(2.21,digits = 2)
    print_comma(0.221121,digits = 2)
    
    
    print_comma(-0.221121,digits = 2)
    print_comma(9.221121,digits = 2)
    print_comma(10.221121,digits = 2)
    
    
    print_comma(x = 'asdad')
    print_comma(x = data[,1])
...
write_excel = function(data,
                       ...
                       create_excel = T,
                       replace_sheet = T,
                       col.names=TRUE, row.names=TRUE, append=T, showNA=TRUE){
  
  if (create_excel){
    if (file.exists(path)){
      message(path, ' already exists, no need to re-create.')
      new_wb<- loadWorkbook(path)
    } else {
      message(path, ' does not exist, need to create fisrt.')
      new_wb = createWorkbook(type="xlsx")
    }
  } else {
    new_wb<- loadWorkbook(path)
  }
  ...
  if (replace_sheet){
    message('worksheet ',sheet, ' is replaced.')
    removeSheet(new_wb, sheetName =sheet)
  }
  saveWorkbook(new_wb, file = path)
  
  write.xlsx(data, 
             path, 
             sheetName = sheet, 
             col.names=col.names, row.names=row.names, append=append, showNA=showNA)
}
